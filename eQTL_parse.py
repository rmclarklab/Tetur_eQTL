...
def args_parser():
    ...
    parser.add_argument("-p", "--p", default = 0.01, type = float, help = "FDR cutoff. ")
...
def iter_check(trans_gene, asso):
    ...
    while True:
        if len(trans_gene) == 0:
            ...
            trans_snp.append(trans_gene[trans_gene['SNP'].isin(snp_sig)][["SNP", "gene", "FDR"]])
...
def main():
    ...
    eQTL_filt = eQTL[(eQTL["FDR"] < p) & (eQTL['SNP'].str.contains("chromosome"))]
