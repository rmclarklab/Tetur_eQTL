...
def args_parser():
    ...
    parser=argparse.ArgumentParser(prog = "PROG", formatter_class = argparse.RawDescriptionHelpFormatter, description="Takes allele-specific read count and genotype data for each sample, return allele-specific read count for all heterozygous regions. \n\
        Usage: python ASE_cis.py -countdir <cnt> -genodir <geno> -gene <gene> -O <prefix>")
...
def search_heter(args):
    ...
    gloc["gene_loc"] = gloc["chromosome"] + ":" + gloc["start"].astype(str)
    ###
    sample_het = list()
    for g in genof:
        pre = g.split(".txt")[0]
        ...
        for c in chrhet:
            ...
            hetgid = list(gloc[(gloc["chromosome"] == c) & (gloc["start"] >= sta) & (gloc["end"] <= end)]["gene_id"])
        ...
        hetcnt = hetcnt[((hetcnt["non_read"] + hetcnt["conflict"])/hetcnt["total_read"] < 0.1) & (hetcnt["total_read"] > 10)]
        ...
        hetcnt["sample"] = pre
        #hetdf = hetcnt.merge(gloc[["gene_id", "gene_loc"]], on = "gene_id", how = "left")
        sample_het.append(hetcnt)
    het_gene = pd.concat(sample_het, axis = 0)
    het_gene = het_gene[["gene_id", "sample", "alt1_read", "alt2_read"]]
    #het_gene = het_gene.rename(columns = {"gene_id":"gene", "alt1_read":"ref"})
    het_gene = het_gene.sort_values(by = ["gene_id", "sample"])
