...
### run: cat Clark_2019/Tetranychus_urticae_v2.gff3 data/Orcae2add_chr_format.gff3 > Tetranychus_urticae_v3.gff3

# translate gff3 to check the quality 
### run: mpiexec -n 6 python code/cds_translate.py -fasta Clark_2019/Tetranychus_urticae_2017.11.21.fasta -gff3 Tetranychus_urticae_v3.gff3 -O v3_translate

# remove any .1 in Tetranychus_urticae_v3.gff3
gff3_v3 = pd.read_table("Clark_2019/Tetranychus_urticae_v3.gff3", sep = "\t", header = None)
...
for index, row in gff3_v3.iterrows():
    ...
    for a in attr_dict:
        if "mRNA" in a:
            attr_dict_new[a] = attr_dict[a].strip(".1")
        else:
...
df_new = pd.DataFrame(row_list)
df_new.to_csv("Tetranychus_urticae_v4.gff3", sep = "\t", index = False, header = False)

# add header line to Tetranychus_urticae_v4.gff3
### run: cat header.gff3 Tetranychus_urticae_v4.gff3 > Tetranychus_urticae_v5.gff3

# sort gff3 (on Adele)
### run: perl ~/Downloads/gff3sort-master/gff3sort.pl Tetranychus_urticae_v5.gff3 > Tetranychus_urticae_v6.gff3 
