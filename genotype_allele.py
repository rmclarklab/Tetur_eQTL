...
def main():
    ...
    outdf.write("chromosome\tposition\tref\talt1\talt2\talt1_count\talt2_count\tothers\n")
    # df = pd.DataFrame(columns = ["chromosome","position", "ref","alt1", "alt2","alt1_count","alt2_count","others"])
    var_df = pd.read_table(var, sep = "\t", header = 0)
    var_list = var_df.values.tolist()
    for i in worker_tasks[rank]:
        row = var_list[i]
        chr = row[0]
        pos = row[1] # 1-based position
        ref = row[2]
        alt1 = row[3]
        alt2 = row[4]
        alt1_count, alt2_count, other = genotype_call(args, chr, pos, alt1, alt2)
        ### for each variant site, write read count
        outdf.write("{0:s}\t{1:d}\t{2:s}\t{3:s}\t{4:s}\t{5:d}\t{6:d}\t{7:d}\n".format(chr, pos, ref, alt1, alt2, alt1_count, alt2_count, other))
        # df = df.append({"chromosome":chr, "position":pos, "ref":ref, "alt1":alt1, "alt2":alt2, "alt1_count":alt1_count, "alt2_count":alt2_count, "others":other}, ignore_index = True)
    outdf.close()
